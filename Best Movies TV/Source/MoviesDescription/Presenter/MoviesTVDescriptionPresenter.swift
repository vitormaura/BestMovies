//
//  MoviesTVDescriptionPresenter.swift
//  Best Movies TV
//
...
protocol MoviesTVDescriptionDelegate: NSObjectProtocol{
...
class MoviesTVDescriptionPresenter {
    
    private weak var delegate:MoviesTVDescriptionDelegate!
    //private var dataBase:FavoriteManager!
    
    init(viewDelegate:MoviesTVDescriptionDelegate) {
...
extension MoviesTVDescriptionPresenter {
    func downloadImage(_ url: String, _ name: String, _ imageView: UIImageView, _ loadingView: LOTAnimationView, _ nameDefault: String){
        ...
        if let url:URL = URL(string: url){
            ...
            imageView.kf.setImage(with: resource, options: [.forceRefresh], completionHandler: { (image, _, _, _) in
